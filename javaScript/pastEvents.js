import { crearTarjetas, aplicarFiltros, selectedCategories, searchText, pepe } from "../modules/modules.js"
const data = {
...
crearTarjetas(tarjetaPadre, data.events, new Date(data.currentDate));
...
for (let i = 0; i < categoriasPE.length; i++) {
    ...
    checkbox.addEventListener('change', function(event) {
        ...
        aplicarFiltros(data, tarjetaPadre);
    });
}

let buscador = document.querySelector("#searchPE");
buscador.addEventListener('input', function(event) {
    searchText.value = event.target.value; // Modifica la propiedad del objeto
    aplicarFiltros(data, tarjetaPadre);
});
